...
for csv = files
    ...
    % plot the figures from 5 Subjects
    figure('Name', plot_name, 'NumberTitle', 'off');
     
    % tile the 3 figures to 2 rows by 2 columns chart
    t = tiledlayout(2,2);
    
    % 1 - Visualize both the orthonormal principal component coefficients of all tests
    vectors = {};
    for i = 1:5
        vectors{end+1} = char(strcat('v_' + string(i)));
    end
    nexttile;
    biplot(coeff(1:5, 1:2), 'scores', score(1:5, 1:2), 'varlabels', vectors);
     
    % data representation in the space of the first 5 principal components 
    [coeff,score,latent,tsquared,explained] = pca(feature_matrix(:, 1:5));
    nexttile;
    
    % 2 - plot the first five components of data in the PCA space
    plot(num_of_tests, dataInPrincipalComponentSpace(:, 1:5));
    xlabel('first five features')
    ylabel('pca space')
    
    % 3 - plot all the values of data in the PCA space
    nexttile;
